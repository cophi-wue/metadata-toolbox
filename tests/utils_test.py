...
class UseCases(unittest.TestCase):
    def setUp(self):
        self.files = Mock(return_value=[FNAME]) # generated by user, e.g. with `glob.glob`
    ...
    def test_reading_processing_writing(self):
        # Step 1: Reading text files and saving content in a list (or any other iterable)
        documents = []
        with patch('builtins.open', mock_open(read_data="This is a document.")) as mock_file:
            for file in self.files.return_value:
        ...
        # Step 3: Getting metadata from filenames
        metadata = []
        for fname in self.files.return_value:
            metadata.append(utils.fname2metadata(fname, '{author}_{title}'))
        
        # Step 4: Modifying metadata and saving with corresponding document in a dictionary
        data = {}
